...
def process_fastq(
    ...
    with (
        pysam.FastxFile(input_r1_fastq) as R1,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Preprocess 3 input files to 2 paired-end reads"
    )
    ...
    parser.add_argument(
        "--bc_raw_count_json", required=True, help="Raw Barcode Count JSON"
    )
    parser.add_argument(
        "--limit", type=int, default=1e9, help="Number of reads to test"
    )
